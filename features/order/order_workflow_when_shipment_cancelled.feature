@order @order_workflow @order_workflow_shipment_cancelled
Feature: Create a new order
  ...
  Scenario: Create order with payment and shipment which still is completed
    ...
    And I apply shipment transition "cancel" to latest order shipment
    Then the order shipping state should be "ready"
