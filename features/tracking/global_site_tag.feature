...
Feature: In order to track ecommerce sales
  ...
  Scenario: Track Checkout Complete
    ...
    Then tracking my order checkout complete with tracker "google-gtag-enhanced-ecommerce" should generate:
      """
      dataLayer.push({"id":%DataObject(order, {"method": "getId"});,"affiliation":24,"total":24,"tax":null,"shipping":0,"currency":"EUR","items":[{"id":%DataObject(orderItem, {"method": "getId"});,"name":"T-Shirt","category":"","price":24,"quantity":1,"currency":"EUR"}]});
      """