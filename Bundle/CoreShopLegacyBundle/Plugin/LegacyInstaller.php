...
/**
 * Class Install
 * @package CoreShop\Bundle\CoreShopLegacyBundle\Plugin
 */
class LegacyInstaller
{
    ...
    {
        \Pimcore::getEventDispatcher()->dispatch('coreshop.install.pre', null, ['installer' => $this]);
        ...
        $this->installAdminTranslations(CORESHOP_PATH.'/Resources/install/translations/admin.csv');

        $this->createImageThumbnails();

        \Pimcore::getEventDispatcher()->dispatch('coreshop.install.post', null, ['installer' => $this]);
    ...
    {
        $file = CORESHOP_PATH . '/Resources/install/sql/'.$fileName.'.sql';
    ...
    public function createClass($className, $updateClass = false)
    {
        ...
        if (!$class || $updateClass) {
            $jsonFile = CORESHOP_PATH . '/Resources/install/class-'.$className.'.json';
    ...
    public function createObjectBrick($name, $jsonPath = null)
    {
        ...
        if ($jsonPath == null) {
            $jsonPath = CORESHOP_PATH . '/Resources/install/fieldcollection-'.$name.'.json';
        }
    ...
    {
        $file = CORESHOP_PATH."/Resources/install/data/objects/$xml.xml";

        if (file_exists($file)) {
            $config = new \Zend_Config_Xml($file);
            $config = $config->toArray();
            $coreShopNamespace = '\\CoreShop\Bundle\CoreShopLegacyBundle\\Model\\'.$namespace;
    ...
    {
        $dataPath = CORESHOP_PATH . '/Resources/install/data/documents';
    ...
    {
        $object = \CoreShop\Bundle\CoreShopLegacyBundle\Model\Order\Workflow::getWorkflowObject();
        $object->save();
    }
    ...
    {
        $file = CORESHOP_PATH . '/Resources/install/data/rules/mailRules.xml';
        ...
        foreach ($objects['MailRule'] as $class => $rule) {
            $existingRule = \CoreShop\Bundle\CoreShopLegacyBundle\Model\Mail\Rule::getByField('name', $rule['name']);

            if ($existingRule instanceof \CoreShop\Bundle\CoreShopLegacyBundle\Model\Mail\Rule) {
                continue;
            }

            $ruleObj = \CoreShop\Bundle\CoreShopLegacyBundle\Model\Mail\Rule::create();
            ...
            foreach ($data as $objectInfo) {
                ...
                $class = '\\CoreShop\Bundle\CoreShopLegacyBundle\\Model\\' . $class;
                $obj = new $class();
                ...
                if (is_subclass_of($obj, '\CoreShop\Bundle\CoreShopLegacyBundle\Model\Mail\Rule\Condition\AbstractCondition')) {
                    $objConditions[] = $obj;
                } elseif (is_subclass_of($obj, '\CoreShop\Bundle\CoreShopLegacyBundle\Model\Mail\Rule\Action\AbstractAction')) {
                    $objActions[] = $obj;
                }
            }
    ...
    {
        if (is_null($path)) {
            $path = CORESHOP_PATH . '/Resources/install/staticroutes.xml';
        }
    ...
    {
        $images = file_get_contents(CORESHOP_PATH . '/Resources/install/thumbnails/images.json');
