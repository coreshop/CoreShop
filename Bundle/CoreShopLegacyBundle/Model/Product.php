...
use CoreShop\Bundle\CoreShopLegacyBundle\Model\Product\AbstractProductPriceRule;
...
use CoreShop\Bundle\CoreShopLegacyBundle\Tool\Service as ToolService;
...
class Product extends Base
{
    ...
    {
        return 'coreshop_product_'.$product->getId().'_price_' . $product->getTaxRate() . '' . \CoreShop\Bundle\CoreShopLegacyBundle\CoreShop::getTools()->getFingerprint();
    }
    ...
    public function getDiscount()
    {
        ...
        /*if(\CoreShop\Bundle\CoreShopLegacyBundle\CoreShop::getTools()->getPricesAreGross()) {
    ...
    public function getPrice($withTax = true, $doCurrencyConvert = true)
    {
        ...
        if ($doCurrencyConvert) {
            return \CoreShop\Bundle\CoreShopLegacyBundle\CoreShop::getTools()->convertToCurrency($netPrice);
        }
    ...
    public function getTaxCalculator(Address $address = null)
    {
        if (is_null($this->taxCalculator)) {
            if (is_null($address)) {
                $cart = \CoreShop\Bundle\CoreShopLegacyBundle\CoreShop::getTools()->getCart();

                $address = $cart->getCustomerAddressForTaxation();

                if (!$address instanceof Address) {
                    $address = Address::create();
                    $address->setCountry(\CoreShop\Bundle\CoreShopLegacyBundle\CoreShop::getTools()->getCountry());
                }
            }
