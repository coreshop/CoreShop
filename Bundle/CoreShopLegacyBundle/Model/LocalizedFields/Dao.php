...
namespace CoreShop\Bundle\CoreShopLegacyBundle\Model\LocalizedFields;
...
/**
 * Class Dao
 * @package CoreShop\Bundle\CoreShopLegacyBundle\Model\LocalizedFields
 */
class Dao extends AbstractDao
{
    ...
    public function delete($deleteQuery = true)
    {
        try {
            if ($deleteQuery) {
                $this->db->delete($this->getLocalizedTableName(),['ooo_id' => $this->model->getObject()->getId()]);

                $validLanguages = Tool::getValidLanguages();
                foreach ($validLanguages as $language) {
                    $queryTable = $this->getQueryTableName().'_'.$language;
                    $this->db->delete($queryTable, ['ooo_id = ?' => $this->model->getObject()->getId()]);
                }
            }
        } catch (\Exception $e) {
            Logger::error($e);
    ...
    public function load()
    {
        ...
        $data = $this->db->fetchAll('SELECT * FROM '.$this->getLocalizedTableName().' WHERE ooo_id = ? AND language IN ('.implode(',', $validLanguages).')',
            [
