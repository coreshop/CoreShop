        ...
        if (array_key_exists($language, $this->getMails())) {
            ...
            if ($mailDocument instanceof Document\Email) {
                if ($model instanceof Model\Messaging\Message) {
                    \CoreShop\Bundle\CoreShopLegacyBundle\Mail::sendMessagingMail($mailDocument, $model, $recipient, $params);
                } elseif ($model instanceof Model\Order) {
                    \CoreShop\Bundle\CoreShopLegacyBundle\Mail::sendOrderMail($mailDocument, $model, false, false, $params);
                } else {
                    \CoreShop\Bundle\CoreShopLegacyBundle\Mail::sendMail($mailDocument, $model, $recipient, $params);
                }
            }
        }
    }
