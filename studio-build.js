...
async function runCommand(command, cwd, silent = false) {
...
async function main() {
  ...
  if (command === 'install') {
    ...
    log('Building all plugins...', colors.yellow);
    
    // Process plugins in parallel for faster builds
    const promises = plugins.map(async (plugin) => {
      const success = await processPlugin(plugin);
      return { plugin: plugin.name, success };
    });
    ...
    results.forEach(result => {
      ...
      log(`  ${result.plugin}: ${status}`, color);
    });
    ...
    if (successful === total) {
      log(`\\n🎉 All ${total} plugins built successfully!`, colors.green);
      process.exit(0);
    } else {
      log(`\\n⚠️  ${successful}/${total} plugins built successfully`, colors.yellow);
      process.exit(1);
    }
  } else if (command === 'dev') {
    ...
    if (bundleName.toLowerCase() === 'all') {
      ...
      const commands = validPlugins.map(plugin => {
        const pluginPath = path.resolve(__dirname, plugin.path);
        return `cd "${pluginPath}" && npm run dev-server`;
      });
      
      const names = validPlugins.map(plugin => plugin.name.replace('Bundle', '')).join(',');
      const colorNames = ['blue', 'green', 'magenta', 'cyan', 'yellow', 'red']
        .slice(0, validPlugins.length).join(',');
      
      // Use concurrently to run all dev servers in parallel  
      const concurrentlyCmd = `npx concurrently ${commands.map(cmd => `"${cmd}"`).join(' ')} --names "${names}" --prefix-colors "${colorNames}" --kill-others-on-fail`;
      ...
      try {
        await installDependencies(plugin);
        await runCommand('npm run dev-server', pluginPath);
      } catch (error) {
