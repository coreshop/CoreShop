...
final class ProductSpecificTierPriceRuleContext implements Context
{
    ...
    /**
     * @Then /^the (specific tier price rule "[^"]+") for (product "[^"]+") should be valid$/
     * @Then /^the (specific tier price rule) should be valid for (product "[^"]+")$/
     */
    public function theSpecificPriceRuleForProductShouldBeValid(ProductSpecificTierPriceRuleInterface $productSpecificPriceRule, ProductInterface $product)
    {
        Assert::true($this->ruleValidationProcessor->isValid($product, $productSpecificPriceRule, $this->shopperContext->getContext()));
    }

    /**
     * @Then /^the (specific tier price rule "[^"]+") for (product "[^"]+") should be invalid$/
     * @Then /^the (specific tier price rule) should be invalid for (product "[^"]+")$/
     */
    public function theSpecificPriceRuleForProductShouldBeInvalid(ProductSpecificTierPriceRuleInterface $productSpecificPriceRule, ProductInterface $product)
    {
