...
final class OrderInvoiceCreationItemsType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ...
            ->add('quantityInvoiced', IntegerType::class, [
                'disabled' => true,
                'required' => false,
                'label' => 'coreshop_invoiced_quantity',
            ])
            ->add('quantity', IntegerType::class, [
                'required' => false,
                'label' => 'coreshop_to_invoice',
            ])
        ;
    }
            ...
            'constraints' => [
                new Callback(static function (mixed $data, ExecutionContextInterface $context): void {
                    ...
                    if ($maxQuantity !== null && $quantity > (int) $maxQuantity) {
                        $context->buildViolation('Quantity ({{ quantity }}) exceeds the maximum invoiceable quantity ({{ max }}).')
