...
export const loadCarriers = async (): Promise<Option[]> => {
  ...
  loadPromise = (async () => {
    try {
      ...
      cachedOptions = list
        .map((r: any) => ({ value: r.id, label: r.identifier ?? r.name ?? String(r.id) }))
...
const CarrierSelectInner: React.FC<{
  value?: number
...
export class DynamicTypeObjectDataCoreShopCarrier extends DynamicTypeObjectDataAbstractSelect {
