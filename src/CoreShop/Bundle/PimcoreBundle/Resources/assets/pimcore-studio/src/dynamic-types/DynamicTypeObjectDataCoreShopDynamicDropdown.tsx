/**
 * CoreShop PimcoreBundle Studio Plugin
 *
 * @copyright  Copyright (c) CoreShop GmbH (https://www.coreshop.com)
 * @license    CoreShop Commercial License (CCL)
 */

import React from 'react'
import { Select } from 'antd'
import {
  DynamicTypeObjectDataAbstractSelect,
  DynamicTypeFieldFilterMultiselect
} from '@pimcore/studio-ui-bundle/modules/element'
import {
  useDynamicDropdownOptions,
  type DynamicDropdownFieldConfig
} from './useDynamicDropdownOptions'

const DynamicDropdownSelect: React.FC<{
  value?: number
  onChange?: (value: number) => void
  disabled?: boolean
  style?: React.CSSProperties
  fieldConfig: DynamicDropdownFieldConfig
  onlyPublished?: boolean
}> = ({ value, onChange, disabled, style, fieldConfig, onlyPublished }) => {
  const { options, loading } = useDynamicDropdownOptions(fieldConfig)

  const filteredOptions = React.useMemo(() => {
    if (onlyPublished) {
      return options.filter(opt => opt.published)
    }
    return options
  }, [options, onlyPublished])

  return (
    <Select
      value={value}
      onChange={onChange}
      loading={loading}
      disabled={disabled}
      style={style}
      showSearch
      optionFilterProp="label"
      allowClear
      options={filteredOptions}
    />
  )
}

export class DynamicTypeObjectDataCoreShopDynamicDropdown extends DynamicTypeObjectDataAbstractSelect {
  readonly id = 'coreShopDynamicDropdown'
  readonly dynamicTypeFieldFilterType = new DynamicTypeFieldFilterMultiselect()

  getObjectDataComponent(props: any): React.ReactElement {
    const { name, noteditable, defaultFieldWidth, ...rest } = props

    const fieldConfig: DynamicDropdownFieldConfig = {
      folderName: props.folderName,
      className: props.className,
      methodName: props.methodName,
      recursive: props.recursive,
      sortBy: props.sortBy
    }

    return (
      <DynamicDropdownSelect
        value={rest.value}
        onChange={rest.onChange}
        disabled={noteditable === true}
        style={{ width: defaultFieldWidth?.width ?? '100%' }}
        fieldConfig={fieldConfig}
        onlyPublished={props.onlyPublished}
      />
    )
  }
}
