...
import { Form, Checkbox, Tabs, Typography, Collapse, Divider } from 'antd'
...
interface StoreOrderMailActionConfig {
...
export const StoreOrderMailAction: React.FC<ActionComponentProps> = ({
  data,
  onChange
}) => {
  const { t } = useTranslation()
  const config = (data ?? {}) as StoreOrderMailActionConfig
  ...
  const handleConfigChange = (key: keyof StoreOrderMailActionConfig, value: boolean) => {
    onChange({
      ...config,
      [key]: value
    })
  ...
  return (
    <>
      <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 16 }}>
        {t('coreshop_store_order_mail_action_description', {
          defaultValue: 'Configure email documents for each store and language. You can optionally attach invoices and shipments to the email.'
        })}
      </Typography.Text>
