...
export const PaymentProviderRuleManager: React.FC = () => {
  ...
  return (
    <EntityTabbedManager<PaymentProviderRule>
      api={paymentProviderRuleApi}
      dragType='coreshop:payment_provider_rule'
      leftRootTitle={t('coreshop_payment_provider_rules', { defaultValue: 'Payment Provider Rules' })}
      localizable
      getTitle={(li, data) => data?.name ?? li?.name ?? `#${li?.id ?? ''}`}
      buildSavePayload={(data) => data}
      onAdd={async () => await new Promise<number>((resolve) => {
        modal.input({
          title: t('coreshop_payment_provider_rule_add', { defaultValue: 'Add Payment Provider Rule' }),
      ...
      renderDetail={(data, setData, ctx) => {
        if (!data) {
          return <div style={{ padding: 12, color: 'var(--ant-color-text-tertiary)' }}>
            {t('coreshop_payment_provider_rule_select', { defaultValue: 'Select a payment provider rule to view details.' })}
          </div>
        }
